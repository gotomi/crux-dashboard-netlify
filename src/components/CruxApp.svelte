<script>
  ...
  function validateForm() {
    ...
    initialData.url.forEach((url, index) => {
      if (url && url.trim() !== '' && !validateUrl(url)) {
        newErrors[`url_${index}`] = 'Please enter a valid URL (e.g., https://example.com)';
        isValid = false;
      }
    });
  ...
  function handleUrlChange(index, value) {
    ...
    if (touched[`url_${index}`] && value && value.trim() !== '') {
      const newErrors = {...errors};
...
<form onsubmit={onSubmit} class="main-form" novalidate>
